console.info("InPuls v34 external: start");

const earlyStatus = document.querySelector("#connection-text");
if (earlyStatus) earlyStatus.textContent = "Загрузка ядра v34…";
...
function showBootError(error) {
  ...
  console.error("InPuls v34 external failed:", error);
...
async function fetchTextWithTimeout(url, timeoutMs = 8_000) {
...
try {
  ...
  const sourceCandidates = [
    `https://raw.githubusercontent.com/veakilag/InPuls/main/app.js?t=${stamp}`,
    `https://cdn.jsdelivr.net/gh/veakilag/InPuls@main/app.js?t=${stamp}`,
    new URL(`./app.js?v=23&raw=v34&t=${stamp}`, location.href).href,
  ];
  ...
  for (const candidate of sourceCandidates) {
    try {
      if (earlyStatus) {
        const host = new URL(candidate).host || "GitHub Pages";
        earlyStatus.textContent = `Загрузка ядра: ${host}…`;
      }
      source = await fetchTextWithTimeout(candidate);
      if (source.includes('const STORAGE_KEYS') && source.includes('class BinanceFeed')) break;
      throw new Error("Получен неполный app.js");
    } catch (error) {
      sourceErrors.push(`${candidate}: ${error?.message || error}`);
      source = "";
    }
  }

  if (!source) {
    throw new Error(`Не удалось загрузить app.js:\n${sourceErrors.join("\n")}`);
  }

  const misses = [];

  const engineUrl = `https://cdn.jsdelivr.net/gh/veakilag/InPuls@main/engine.js?t=${stamp}`;
  const chartUrl = `https://cdn.jsdelivr.net/gh/veakilag/InPuls@main/chart.js?t=${stamp}`;
  const orderbookUrl = new URL("./orderbook-v34.js?v=34", location.href).href;
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    ...
    `// InPuls v34: ядро загружено вне зоны Service Worker.`,
  ...
  source = `console.info("InPuls v34 app", { misses: ${JSON.stringify(misses)} });\n${source}`;

  const blobUrl = URL.createObjectURL(new Blob([source], { type: "text/javascript" }));
  try {
    await import(blobUrl);
    console.info("InPuls v34 external: app started", { misses });
  } finally {
