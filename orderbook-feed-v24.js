import { applyDepthUpdates, depthView, normalizeMarketTrade } from "./orderbook.js?legacy=23";
...
export class OrderBookFeed {
  ...
  async #loadDepthSnapshot(generation) {
    ...
    this.onStatus({ state: "online", text: "LIVE 100ms · DEEP" });
  }
  ...
  #startDepth(generation) {
    ...
    socket.addEventListener("open", () => {
      ...
      this.onStatus({ state: "loading", text: "Синхронизация глубины" });
    ...
    socket.addEventListener("message", (event) => {
      ...
      this.onStatus({ state: "online", text: rate === "500ms" ? "LIVE 500ms · DEEP" : "LIVE 100ms · DEEP" });
    });
