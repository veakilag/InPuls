...
# 1) Extend the existing research context module with relational/local structure.
...
// V6.2 is deliberately relational and descriptive. It does not add another
// aggregate trading score. The same candidate can be the nearest execution
// boundary while a farther candidate is the stronger structural boundary.
// Both views are exposed so later outcome data can tell us which relationship
// matters for breakout/cascade research instead of hard-coding the answer now.
export function buildLocalStructureResearchContext(levelContexts, {
  ...
  return Object.freeze({
    ...
    strongestBracket: bracketMetrics(
      strongestLow,
      strongestHigh,
      local,
      resolvedCurrentPrice,
    ...
    // No direction, probability or buy/sell recommendation is produced here.
    researchOnly: true,
  });
}
...
# 2) Runtime: import and print the relational structure block.
...
# 3) Extend tests with relational behavior.
...
# 4) Keep the standalone bundle functional now that runtime imports v8 context.
