...
async function probeChartModal(socket) {
  const evaluation = await send(socket, "Runtime.evaluate", {
    expression: `(async () => {
      ...
      const annotationModule = await import('./signal-lab-v3-full-chart.js?v=26-118-signal-lab-30d-history-v1');
      const now = Date.now();
      ...
      if (button) {
        ...
        const modalModule = await import('./signal-lab-chart-modal.js?v=26-118-signal-lab-30d-history-v1');
