console.info("InPuls v33 direct: start");

const earlyStatus = document.querySelector("#connection-text");
if (earlyStatus) earlyStatus.textContent = "Запуск v33…";
...
function showBootError(error) {
  ...
  console.error("InPuls v33 direct failed:", error);
...
try {
  const rawUrl = new URL(`./app.js?v=23&raw=v33&t=${Date.now()}`, location.href);
  ...
  const orderbookUrl = new URL("./orderbook.js?v=33", location.href).href;
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    ...
    `// InPuls v33: приложение запущено напрямую без Service Worker.`,
  ...
  source = `console.info("InPuls v33 app", { misses: ${JSON.stringify(misses)} });\n${source}`;

  const blobUrl = URL.createObjectURL(new Blob([source], { type: "text/javascript" }));
  try {
    await import(blobUrl);
    console.info("InPuls v33 direct: app started", { misses });
  } finally {
