...
export function invertBookScaleWheelDelta(deltaY) {
  const value = Number(deltaY);
  return Number.isFinite(value) ? -value : 0;
}

export function rawTapeBubbleDiameter(strength, viewportWidth) {
  const normalized = clampTape(strength, 0, 1.9);
  const width = Math.max(80, Number(viewportWidth) || 80);
  const maximum = Math.min(44, Math.max(12, width * .055));
  return clampTape(3 + Math.pow(normalized, 1.08) * 18, 3, maximum);
}

export function aggregateTapeBlockSize(strength, labelWidth, viewportWidth) {
  const normalized = clampTape(strength, 0, 1.9);
  const width = Math.max(80, Number(viewportWidth) || 80);
  return {
    height: clampTape(10 + normalized * 10, 10, 29),
    width: clampTape(
      (Number(labelWidth) || 0) + 10 + normalized * 8,
      22,
      Math.min(112, width * .38),
    ),
  };
}
...
function createTapeStrengthScale(values) {
  ...
  const low = Math.max(1, sorted[Math.floor((sorted.length - 1) * .10)] || 1);
  const high = Math.max(low + 1, sorted[Math.floor((sorted.length - 1) * .82)] || low + 1);
...
function drawTapeCard(card) {
  ...
  for (let index = 0; index < drawItems.length; index += 1) {
    ...
    if (state.mode === "raw") {
      ...
      const diameter = rawTapeBubbleDiameter(strength, rect.width);
      const x = clampTape(
    ...
    const label = formatTapeUsd(item.quote);
    const measured = context.measureText(label).width;
    const block = aggregateTapeBlockSize(strength, measured, rect.width);
    const { height, width } = block;
...
function drainTapeIngest() {
  ...
  for (const [symbol, pending] of tapePendingBySymbol) {
    ...
    if (pending.resume) {
      // Один компактный merge на символ вместо десятков тяжёлых проходов
      // после возвращения из фоновой вкладки.
...
function acceptTapeData(event) {
  ...
  if (detail.resume) {
    // На возврате важнее мгновенно показать актуальный поток, чем
    // догонять каждую сделку, накопленную во время фоновой вкладки.
...
function relayCorrectedBookScaleWheel(event) {
  if (event?.__inpulsBookScaleDirectionFixed) return;
  if (!(event.ctrlKey || event.metaKey)) return;
  const target = event.target;
  const stage = target?.closest?.(".orderbook-stage");
  if (!stage || target.closest?.(".orderbook-tape")) return;
  ...
  const corrected = new WheelEvent("wheel", {
    ...
    deltaY: invertBookScaleWheelDelta(event.deltaY),
  ...
  Object.defineProperty(corrected, "__inpulsBookScaleDirectionFixed", { value: true });
...
function installOrderBookRuntime() {
  ...
  document.addEventListener("wheel", relayCorrectedBookScaleWheel, { capture: true, passive: false });
