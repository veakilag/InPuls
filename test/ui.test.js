...
test("browser entry points keep user version v23 and identify the current release build", async () => {
  ...
  assert.match(html, /inpuls-build" content="26-49-density-trades-correlation-v1"/);
  assert.match(worker, /inpuls-26-49-density-trades-correlation-v1/);
...
test("browser entry points carry a restrictive CSP and reset scripts stay external", async () => {
  ...
  assert.match(pages[3], /refresh\.js\?v=26-49-density-trades-correlation-v1/);
  assert.match(pages[4], /reset\.js\?v=26-49-density-trades-correlation-v1/);
});
