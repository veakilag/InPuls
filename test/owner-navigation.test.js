...
test("owner dashboard stays unlinked, local-only and exposes no destructive history controls", async () => {
  ...
  assert.match(worker, /owner-signal-lab-guard\.js\?v=26-59-signal-lab-candles-explanation-v1/);
  assert.match(worker, /owner-signal-lab\.js\?v=26-59-signal-lab-candles-explanation-v1/);
});
