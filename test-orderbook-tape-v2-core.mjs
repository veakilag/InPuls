...
test("Flow Workspace cache and reset page point to water runtime", () => {
  ...
  assert.match(orderbook, /orderbook-flow-workspace\.js\?v=26-114-raw-series-execution-candles-v1/);
  assert.match(serviceWorker, /inpuls-26-91-runtime-boot-cache-feed-v1/);
  assert.match(serviceWorker, /app\.js\?v=26-114-raw-series-execution-candles-v1/);
  assert.match(serviceWorker, /canvas-comfort-preview\.js\?v=26-114-raw-series-execution-candles-v1/);
  assert.match(serviceWorker, /orderbook\.js\?v=26-114-raw-series-execution-candles-v1/);
  assert.match(serviceWorker, /render-scheduler\.js\?v=render-scheduler-v1/);
  assert.match(serviceWorker, /orderbook-worker\.js\?v=26-114-raw-series-execution-candles-v1/);
  ...
  assert.match(serviceWorker, /orderbook-flow-workspace\.js\?v=26-114-raw-series-execution-candles-v1/);
...
test("production TAPE accepts only live packets and starts from an empty frame", () => {
  assert.match(orderbook, /if \(!detail\?\.replace && !detail\?\.seriesReplace && !detail\?\.live\) return;/);
  assert.match(orderbook, /const carriesData = Boolean\(detail\?\.live \|\| detail\?\.replace \|\| detail\?\.seriesReplace\)/);
});
