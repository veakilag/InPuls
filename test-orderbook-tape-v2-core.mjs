...
test("Flow Workspace cache and reset page point to water runtime", () => {
  assert.match(orderbook, /inpuls-orderbook-runtime-26-72-water-tape-fast-v1/);
  ...
  assert.match(serviceWorker, /orderbook\.js\?v=26-72-water-tape-fast-v1/);
  assert.match(serviceWorker, /render-scheduler\.js\?v=render-scheduler-v1/);
  assert.match(serviceWorker, /orderbook-worker\.js\?v=26-72-water-tape-fast-v1/);
  ...
  assert.match(serviceWorker, /orderbook-flow-workspace\.js\?v=26-72-water-tape-fast-v1/);
  assert.match(resetPage, /Resume v2/);
  assert.match(resetPage, /reset\.js\?v=26-72-water-tape-fast-v1/);
  assert.match(resetScript, /sw\.js\?v=\$\{BUILD\}/);
  assert.match(resetScript, /26-72-water-tape-fast-v1/);
});
