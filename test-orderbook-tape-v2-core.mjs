...
test("Flow Workspace cache and reset page point to the new runtime", () => {
  assert.match(orderbook, /inpuls-orderbook-runtime-26-50-signal-memory-contract-v1/);
  assert.match(orderbook, /orderbook-flow-workspace\.js\?v=26-50-signal-memory-contract-v1/);
  ...
  assert.match(resetPage, /reset\.js\?v=26-50-signal-memory-contract-v1/);
  assert.match(resetScript, /sw\.js\?v=\$\{BUILD\}/);
  assert.match(resetScript, /26-50-signal-memory-contract-v1/);
});
