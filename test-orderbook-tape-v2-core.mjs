...
test("Flow Workspace cache and reset page point to water runtime", () => {
  assert.match(orderbook, /inpuls-orderbook-runtime-26-77-tiger-zero-ms-agg-v1/);
  assert.match(orderbook, /orderbook-flow-workspace\.js\?v=26-77-tiger-zero-ms-agg-v1/);
  assert.match(serviceWorker, /inpuls-26-77-tiger-zero-ms-agg-v1/);
  assert.match(serviceWorker, /orderbook\.js\?v=26-77-tiger-zero-ms-agg-v1/);
  assert.match(serviceWorker, /render-scheduler\.js\?v=render-scheduler-v1/);
  assert.match(serviceWorker, /orderbook-worker\.js\?v=26-77-tiger-zero-ms-agg-v1/);
  ...
  assert.match(resetPage, /reset\.js\?v=26-77-tiger-zero-ms-agg-v1/);
  assert.match(resetScript, /sw\.js\?v=\$\{BUILD\}/);
  assert.match(resetScript, /26-77-tiger-zero-ms-agg-v1/);
});
